...
function isPalindrome(str) {
  return (
    str
      ...
      .reverse()
      .join('')
      .toLowerCase() === str.replace(/[\s.,!?%]/g, '').toLowerCase()
  );
}
...
function findLongestWord(sentence) {
  ...
  let arrayWordBase = arrayWords[0];
  for (let i = 0; i < arrayWords.length; i += 1) {
    if (arrayWords[i].length > arrayWordBase.length) {
      arrayWordBase = arrayWords[i];
    }
  }
  return arrayWordBase;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (char) => alphabetToRot13[alphabet.indexOf(char)]
  );
}
...
function getCardId(value) {
  const firstChar = value.substring(0, value.length - 1);
  const arrLetters = [
    'A',
    '2',
    '3',
  ...
  const arrSymbols = ['♣', '♦', '♥', '♠'];
  return arrLetters.indexOf(firstChar) + arrSymbols.indexOf(lastChar) * 13;
}
